...
exports.getPostDetails = async (req, res) => {
    ...
    const loggedInUserId = req.userId; // 로그인된 유저의 ID
    ...
    const sqlQuery = `
        SELECT 
            ...
            p.post_date, 
            p.language, 
            p.user_id AS post_user_id,  -- 포스트 작성자의 user_id
            p.post_result,
            u.user_nickname,
            u.user_image,
            u.user_contribute,
    ...
    try {
        ...
        // 응답에 로그인된 유저의 ID와 포스트 작성자의 ID를 추가
        res.json({
            ...results[0],
            loggedInUserId, // 로그인된 유저의 ID
            postUserId: results[0].post_user_id // 포스트 작성자의 ID
        });
    } catch (err) {
        console.error('데이터베이스 오류:', err);
        res.status(500).json({ error: '데이터베이스 오류' });
